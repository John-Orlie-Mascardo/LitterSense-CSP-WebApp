...
test("RFID heartbeat reports entry without counting a visit and exit clears active state", () => {
  const args = {
    deviceId: "rfid-test", configToken: "cfg_test", recordedEvents: [], ignoredEvents: [],
    now: new Date("2026-09-08T02:00:10Z"),
    liveSensors: { sessionActive: true, activeRfidHex: "300833B2DDD9014000000001",
      activeSessionStartMs: Date.parse("2026-09-08T02:00:00Z"), activeSessionDurationMs: 10000 },
  };
  const active = buildDeviceSensorSnapshot(args);
  assert.equal(active.sessionActive, true);
  assert.equal(active.activeSessionDurationMs, 10000);
  assert.equal(active.currentSessionStatus, "IN_PROGRESS");
  assert.equal(active.completedSessionCount, 0);
  const ended = buildDeviceSensorSnapshot({ ...args, previous: active, liveSensors: { sessionActive: false } });
  assert.equal(ended.sessionActive, false);
  assert.equal(ended.activeRfidHex, "");
  assert.equal(ended.activeSessionStartMs, null);
  assert.equal(ended.currentSessionStatus, "IDLE");
});
