...
test("RFID firmware payload preserves duration, matches full EPC and retries the same session", () => {
  const payload = { deviceId: "AA:BB:CC:DD:EE:FF", sessionActive: false, events: [{
    eventId: "abcdef0123456789_1000_11000", status: "NORMAL",
    rfidHex: "300833B2DDD9014000000001", durationMs: 10000, endedAtMs: 1788832800000,
  }] };
  const first = normalizeSensorSyncRequest(payload, new Date("2026-09-08T02:01:00Z")).events[0];
  const retry = normalizeSensorSyncRequest(payload, new Date("2026-09-08T02:02:00Z")).events[0];
  assert.equal(first.durationSecs, 10);
  assert.equal(Date.parse(first.endedAt) - Date.parse(first.startedAt), 10000);
  assert.equal(findCatIdByRfid([["cat-1", {rfidTag: payload.events[0].rfidHex}]], "", first.rfidHex), "cat-1");
  assert.equal(buildSessionDocumentId("cfg_device", first), buildSessionDocumentId("cfg_device", retry));
  assert.equal(normalizeSensorSyncRequest({events: [], sessionActive: true}).events.length, 0);
});
