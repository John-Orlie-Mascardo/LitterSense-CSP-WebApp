...
test("uses configured LAN origin when browser origin is localhost", () => {
  assert.equal(
    getHardwareReachableOrigin(
      "http://localhost:3000",
...
test("keeps LAN browser origin for hardware provisioning", () => {
  assert.equal(
    getHardwareReachableOrigin(
      "http://192.168.68.106:3000",
      "http://192.168.68.200:3000",
    ),
    "http://192.168.68.106:3000",
  );
});

test("falls back to browser origin when no LAN origin is configured", () => {
  assert.equal(getHardwareReachableOrigin("http://localhost:3000"), "http://localhost:3000");
});
