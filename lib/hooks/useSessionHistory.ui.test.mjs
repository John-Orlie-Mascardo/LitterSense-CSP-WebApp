 ...
 *
 * DONE: inclusive range query, deterministic order, cursor, batch limit, safe error copy
 * PLACEHOLDER: none
 *
...
test("history uses inclusive date bounds, deterministic ordering, cursors, and limits", () => {
  assert.match(source, /where\("date", ">=", filters\.startDate\)/);
  assert.match(source, /where\("date", "<=", filters\.endDate\)/);
  assert.match(source, /orderBy\("date", filters\.sort\)/);
  assert.match(source, /orderBy\(documentId\(\), filters\.sort\)/);
